...
export default function CreateCampaignPage() {
  ...
  const isEditMode = location.pathname.startsWith('/modifier-campagne');
  ...
  const isStepValid = (s: number): boolean => {
    switch (s) {
      ...
      case 3:
        ...
        return information.trim().length > 0 && rules.some((r) => r.trim().length > 0) && documents.length > 0;
      default:
        return true;
    }
  };
  ...
  return (
    <div className="h-screen text-white flex flex-col overflow-y-auto" style={{ backgroundColor: '#050404' }}>
      <div className="max-w-[1440px] mx-auto px-4 lg:px-8 py-6 lg:py-10 pb-32">
        <div className="flex items-center gap-4 mb-10">
          <button
            onClick={() => setShowExitModal(true)}
            className="lg:hidden flex items-center justify-center w-10 h-10 rounded-full hover:bg-white/10 transition-colors shrink-0"
          >
            <ArrowLeft className="w-5 h-5 text-white" />
          </button>
          <div className="flex-1">
            <h1 className="text-xl font-bold tracking-tight">{isEditMode ? 'Modifier votre campagne' : 'Creation de votre nouvelle campagne'}</h1>
      ...
      {showDeleteModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center px-4" style={{ backgroundColor: 'rgba(0,0,0,0.7)', backdropFilter: 'blur(8px)' }}>
          ...
          >
            ...
            <p className="text-sm text-white/50 mb-8 leading-relaxed">
              Cette action est irréversible. Le brouillon de cette campagne sera définitivement supprimé.
            </p>
