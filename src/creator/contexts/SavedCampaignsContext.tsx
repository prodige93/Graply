...
/**
 * Liste « Enregistré » **créateur uniquement** : colonne `profiles.saved_campaign_ids`.
 * L’app entreprise utilise `enterprise_saved_campaign_ids` (autre contexte) — ne pas fusionner les deux.
 */

/** Comparaison stable des UUID (évite incohérences entre vues de la même app). */
function normId(id: string): string {
  return id.trim().toLowerCase();
}

function previewStorageKey(uid: string): string {
  return `graply_creator_saved_previews_${uid}`;
}
...
interface SavedCampaignsContextType {
  savedIds: string[];
  /** À l’ajout, passer `preview` (données de la carte) pour affichage fiable dans Enregistré. */
...
export function SavedCampaignsProvider({ children }: { children: ReactNode }) {
  ...
  const loadSaved = useCallback(async (uid: string) => {
    const { data } = await supabase
      ...
      .eq('id', uid)
      .maybeSingle();
    const next = data?.saved_campaign_ids;
  ...
  async function persistSaved(uid: string, newIds: string[]) {
    const { error } = await supabase
      ...
      .eq('id', uid);
    if (error) {
      console.error('[SavedCampaigns] persist failed', error);
      await loadSaved(uid);
    }
  }
