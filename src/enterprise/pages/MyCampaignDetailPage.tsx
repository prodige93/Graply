...
import { ArrowLeft, Megaphone, Users, Eye, DollarSign, Calendar, Pause, Play, Trash2, Pencil, TrendingUp } from 'lucide-react';
...
export default function MyCampaignDetailPage() {
  ...
  const backTo = (location.state as { from?: string } | null)?.from ?? '/mes-campagnes';
  const { updateCampaignStatus } = useMyCampaigns();
  ...
  if (loading) {
    return (
      <div className="text-white min-h-screen" style={{ backgroundColor: '#050404' }}>
        ...
        <div className="max-w-5xl mx-auto px-4 lg:px-10 py-8 space-y-6">
          <div className="h-7 w-48 rounded-lg animate-pulse" style={{ background: 'rgba(255,255,255,0.08)' }} />
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            {[...Array(4)].map((_, i) => (
              <div key={i} className="h-24 rounded-2xl animate-pulse" style={{ background: 'rgba(255,255,255,0.055)' }} />
            ))}
          </div>
          <div className="h-64 rounded-2xl animate-pulse" style={{ background: 'rgba(255,255,255,0.055)' }} />
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="text-white relative" style={{ backgroundColor: '#050404' }}>
      {isPaused && (
        <div
        ...
        >
          ...
          <button
            ...
            style={{ background: 'rgba(255,255,255,0.08)', backdropFilter: 'blur(12px)', WebkitBackdropFilter: 'blur(12px)', border: '1px solid rgba(255,255,255,0.18)', color: '#ffffff' }}
          >
      ...
      <div className="max-w-6xl mx-auto px-6 -mt-16 relative z-10 pb-16">
        <div className="flex flex-col sm:flex-row sm:items-start gap-4 mb-6">
          ...
          <div className="flex items-center gap-2 shrink-0 sm:mt-2">
            <button
              onClick={() => navigate(`/modifier-campagne/${campaign.id}`, { state: { from: `/ma-campagne/${campaign.id}` } })}
              className="flex items-center gap-2 h-9 px-4 rounded-full transition-all duration-200 hover:brightness-125"
              style={{
                background: 'rgba(255,255,255,0.08)',
                backdropFilter: 'blur(12px)',
                WebkitBackdropFilter: 'blur(12px)',
            ...
            <button
              onClick={handleTogglePause}
              disabled={pauseLoading}
              className="flex items-center gap-2 h-9 px-4 rounded-full transition-all duration-200 hover:brightness-110 disabled:opacity-60"
              style={isPaused ? {
            ...
            >
              {isPaused ? (
                <>
                  ...
                  <span className="text-xs font-semibold text-white">Reprendre</span>
                </>
              ) : (
                <>
                  <Pause className="w-3.5 h-3.5 text-white/50" />
                  <span className="text-xs font-semibold text-white/50">En pause</span>
                </>
              )}
            </button>
            <button
              onClick={() => setConfirmDelete(true)}
              className="flex items-center justify-center w-9 h-9 rounded-full transition-all duration-200 hover:brightness-125"
              style={{
        ...
        >
          <div className="flex items-center gap-4">
            ...
            <div>
              <p className="text-sm font-bold text-white">Vérification vidéo créateur</p>
              <p className="text-xs text-white/35 mt-0.5">Vidéos en attente de vérification</p>
            </div>
          </div>
          <div className="flex items-center gap-3">
            <div
            ...
            <button
              onClick={() => navigate(`/ma-campagne/${campaign.id}/verifications`, { state: { from: `/ma-campagne/${campaign.id}` } })}
        ...
        >
          ...
          <div className="flex items-center justify-between mb-3">
            <span className="text-sm font-semibold text-white">$0.00 dépensé</span>
        ...
        <div
          ...
          style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)' }}
        >
          <h2 className="text-sm font-bold text-white uppercase tracking-wider mb-5">Créateurs</h2>
          <div className="flex flex-col items-center justify-center py-12">
            <Users className="w-10 h-10 text-white/10 mb-3" />
            <p className="text-sm text-white/30 font-medium">Aucun créateur pour le moment</p>
            <p className="text-xs text-white/15 mt-1">Les créateurs qui postulent apparaîtront ici</p>
          </div>
        </div>
          ...
          <div className="p-5 pb-2">
              ...
              <div className="hidden sm:flex items-center gap-2 shrink-0 ml-auto">
                ...
                <div>
                  <p className="text-[9px] text-white/25 uppercase tracking-wider font-medium leading-none mb-0.5">
                    {dashMetric === 'views' ? 'Total vues' : 'Total dépensé'}
            ...
            <div className="flex sm:hidden items-center justify-between gap-3 mb-4">
              <div className="flex items-center gap-2 shrink-0">
                ...
                {dashMetric === 'earned' && !activePlatform && (
                  <span className="text-[11px] text-white/20 font-medium self-end mb-0.5">/ {totalBudget > 0 ? `$${totalBudget.toLocaleString()}` : campaign.budget}</span>
                )}
              </div>
